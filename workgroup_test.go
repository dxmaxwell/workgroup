...
func doWorkWithLimit(g *Group, n int, limit int, t *testing.T) {
    ...
    ForLimit(g, n, limit, func(idx int) error {
        RandomSleep()
        done <- idx
        return nil
    })
...
func TestLoopWorkWithErrors0(t *testing.T) {
    doWorkWithErrors(WithErrors(), 1, 10000, t)
    doWorkWithErrors(WithErrors(), 10, 10000, t)
    doWorkWithErrors(WithErrors(), 100, 10000, t)
}
